...
func ConfigFromParsed(pConf *service.ParsedConfig) (conf OldConfig, err error) {
	...
	conf.NegotiateEnabled, _ = negotiate.FieldBool(hcFieldNegotiateEnabled)
	...
	if negotiate.Contains(hcFieldNegotiateUser) {
		...
		negotiateAuthOptions.User = &option.User{
			Domain:   userDomain,
			Name:     userName,
	...
	if negotiate.Contains(hcFieldNegotiateKerberos) {
		...
		negotiateAuthOptions.Kerberos = &option.Kerberos{
			ConfigFilePath: filePath,
			CCName:         ccname,
		}
	}

	conf.NegotiateAuthOptions = negotiateAuthOptions
