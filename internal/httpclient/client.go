...
func NewClientFromOldConfig(conf OldConfig, mgr *service.Resources, opts ...RequestOpt) (*Client, error) {
	...
	if tlsClientConfig != nil || proxy != nil {
		...
		if conf.NegotiateEnabled {
			h.client.Transport = spnego.NewRoundTripper(h.client.Transport, conf.NegotiateApi, conf.NegotiateAuthOptions)
		}
	} else if conf.NegotiateEnabled {
		h.client.Transport = spnego.NewRoundTripper(http.DefaultTransport, conf.NegotiateApi, conf.NegotiateAuthOptions)
	}
